...
# 💡 [핵심 개선] DB 스키마 정의 (PC_CATEGORY 및 외래 키)
def ensure_db_schema(conn):
    """ DB에 필요한 4개 테이블의 스키마를 보장하며, PC_CATEGORY의 TEXT PK에 맞춥니다. """
    cursor = conn.cursor()
    try:
        # 4. PC_CATEGORY 테이블 (먼저 생성)
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS PC_CATEGORY (
                PC_Code TEXT PRIMARY KEY,
                PC_Name TEXT
            );
        """)
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS PRODUCT (
                ...
                ProductModel_Name TEXT,
                ProductModel_Suffix TEXT NOT NULL
            );
        """)
        
        # 2. TEST_SPECIFICATION 테이블
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS TEST_SPECIFICATION (
                TestItemName TEXT PRIMARY KEY,
                MinLimit REAL NOT NULL,
                MaxLimit REAL NOT NULL
            );
        """)
        
        # 3. TEST_HISTORY 테이블 (PcbMaxIrPwr_Value 대신 PC_Code를 사용하여 PC_CATEGORY와 연결)
        # 3. TEST_HISTORY 테이블 생성 (없을 경우)
        # 🚨 [수정] 모든 측정값 필드를 다시 포함해야 합니다.
        measure_cols = [f'{col}_Value REAL' for col in BASE_MEASUREMENTS]
        measure_cols_str = ',\n'.join(measure_cols)
        
        cursor.execute(f"""
            CREATE TABLE IF NOT EXISTS TEST_HISTORY (
                History_PK INTEGER PRIMARY KEY AUTOINCREMENT,
                Original_Local_TestID INTEGER,
                ...
                FOREIGN KEY (SNumber) REFERENCES PRODUCT(SNumber),
                FOREIGN KEY (PC_Code) REFERENCES PC_CATEGORY(PC_Code) -- 💡 외래 키 연결
            );
        """)

        # 🚨 [자동 스키마 업데이트 로직]
        # PRAGMA table_info로 현재 컬럼 목록을 가져옵니다.
        cursor.execute("PRAGMA table_info(TEST_HISTORY)")
        existing_columns = {col[1] for col in cursor.fetchall()}
        
        # 1. PC_Code 컬럼 추가
        if 'PC_Code' not in existing_columns:
            cursor.execute("ALTER TABLE TEST_HISTORY ADD COLUMN PC_Code TEXT")
            st.info("💡 스키마 업데이트: TEST_HISTORY에 PC_Code 컬럼이 추가되었습니다.")
            
        # 2. 측정값 컬럼 추가 (이전 스키마에 이 필드가 누락되었을 가능성)
        for col_name in [f'{col}_Value' for col in BASE_MEASUREMENTS]:
            if col_name not in existing_columns:
                try:
                    cursor.execute(f"ALTER TABLE TEST_HISTORY ADD COLUMN {col_name} REAL")
                    st.info(f"💡 스키마 업데이트: TEST_HISTORY에 {col_name} 컬럼이 추가되었습니다.")
                except Exception as e:
                    st.warning(f"컬럼 추가 오류 ({col_name}): {e}")

        conn.commit()
        return True
    except Exception as e:
        st.error(f"DB 스키마 생성/업데이트 오류: {e}")
        conn.rollback()
        return False

    #     # 🚨 [추가] TEST_HISTORY에 PC_Code 컬럼이 없을 경우 추가하는 로직
    #     # 이는 이전 단계의 "table TEST_HISTORY has no column named PC_Code" 오류를 방지합니다.
    #     try:
    #         # PRAGMA table_info로 TEST_HISTORY 테이블의 현재 컬럼 목록을 확인
    #         cursor.execute("PRAGMA table_info(TEST_HISTORY)")
    #         columns = [col[1] for col in cursor.fetchall()]
            
    #         if 'PC_Code' not in columns:
    #             cursor.execute("ALTER TABLE TEST_HISTORY ADD COLUMN PC_Code TEXT")
    #             st.info("💡 DB 스키마 업데이트: TEST_HISTORY 테이블에 PC_Code 컬럼을 추가했습니다.")
            
    #     except Exception as e:
    #         # ALTER TABLE 자체에서 오류가 날 경우 (예: SQLite 버전이 낮을 경우)
    #         st.warning(f"DB 스키마 자동 업데이트 실패 (PC_Code): {e}")

    #     conn.commit()
    #     return True
    # except Exception as e:
    #     st.error(f"DB 스키마 생성 오류: {e}")
    #     return False
...
# (get_total_count_for_query 및 get_default_dates 함수는 변경 없이 유지)
...
# (get_db_status 및 delete_db_data 함수는 변경 없이 유지)
...
def delete_db_data(conn, year, suffix, week):
    """ TEST_HISTORY와 PRODUCT 테이블에서 조건에 맞는 데이터를 900개 단위로 삭제합니다. """
    try:
        cursor = conn.cursor()
        
        # 1. 삭제 대상 SNumber 목록 조회 (전체 목록)
        find_sn_query = f"""
        ...
        all_delete_sns = df_sn['SNumber'].tolist()
        
        if not all_delete_sns:
            return 0, 0, "삭제 조건에 맞는 SNumber가 없습니다."

        chunk_size = 900 # SQLite 변수 제한(999)보다 작은 안전한 청크 크기
        deleted_history_total = 0
        deleted_product_total = 0
        
        # 2. 청크 단위로 분할하여 삭제 실행
        for i in range(0, len(all_delete_sns), chunk_size):
            chunk_sns = all_delete_sns[i:i + chunk_size]
            placeholders = ','.join(['?'] * len(chunk_sns))
            
            # 2-1. TEST_HISTORY에서 삭제
            delete_history_query = f"""
            DELETE FROM TEST_HISTORY
            WHERE TestWeek = '{week}' AND SNumber IN ({placeholders});
            """
            cursor.execute(delete_history_query, chunk_sns)
            deleted_history_total += cursor.rowcount
            
            # 2-2. PRODUCT 테이블에서 삭제 (다른 TEST_HISTORY 기록이 없는 제품만 삭제)
            delete_product_query = f"""
            DELETE FROM PRODUCT
            WHERE SNumber IN ({placeholders})
              AND SNumber NOT IN (SELECT SNumber FROM TEST_HISTORY);
            """
            cursor.execute(delete_product_query, chunk_sns)
            deleted_product_total += cursor.rowcount
        
        conn.commit()
        return deleted_history_total, deleted_product_total, None
...
# 💡 스키마 변경에 따라 기본 쿼리도 PC_Code를 사용하도록 수정
...
SELECT * FROM (
    WITH All_Test_Results AS (
        ...
        SELECT
            T1.Original_Local_TestID, T1.SNumber, T1.PcbStartTime, T1.PcbStopTime, T1.PC_Code, 
            T3.PC_Name, -- 💡 PC_CATEGORY에서 PC_Name을 가져옵니다.
            ...
            -- 각 TestItemName에 대한 Spec_Result_Detail을 계산합니다. (로직은 원본 유지)
            CASE
        ...
        -- PC_CATEGORY 조인
        LEFT JOIN PC_CATEGORY AS T3 ON T1.PC_Code = T3.PC_Code 
    ...
    SELECT
        ...
        ATR.PC_Code, ATR.PC_Name, -- 💡 PC_Code와 PC_Name 추가
...
selected_mode = st.radio( 
    ...
    index=['2. DB 조회 및 분석', '1. 데이터 저장/추가 (CSV Insert)', '3. 주차별 DB 데이터 삭제'].index(st.session_state.mode),
    horizontal=False
)
...
if st.session_state.mode == '1. 데이터 저장/추가 (CSV Insert)':
    ...
    with col_week:
        selected_week = st.text_input("1. 업로드할 데이터 주차  입력 (예: 44W)", value="44W", key='input_week_select')
    ...
    if uploaded_files:
        ...
        # 파일 목록 순회하며 데이터 처리 (캐시 활용)
        for file in uploaded_files:
            file_key = file.name
            
            # 파일이 세션 상태에 없거나 주차가 변경되면 재처리 시작
            if file_key not in st.session_state.uploaded_files_data or st.session_state.uploaded_files_data[file_key].get('test_week') != selected_week:
        ...
        if st.button("DB에 데이터 저장/추가 실행", key='final_save_btn'):
            ...
            for file_key, data in st.session_state.uploaded_files_data.items():
                ...
                if success:
                    st.success(f"✔️ {file_key} ({data['test_week']}) 파일 데이터 {added_rows}행 DB에 성공적으로 추가. (PC_CATEGORY, PRODUCT 중복 제외됨)")
...
# ==============================================================================
    ...
    df_status = get_db_status(conn)
    st.subheader(f"DB에 저장된 주차별 상세 데이터 현황 (현재 연결 파일: {DB_FILE})")
    if not df_status.empty:
        st.dataframe(df_status, use_container_width=True)
        # 💡 모드 전환 시 초기 쿼리 실행
        if st.session_state.custom_query_result is None:
            try:
                # Limit 100을 추가하여 안전하게 실행
                initial_query = re.sub(r'LIMIT\s+\d+\s*;?\s*$', '', st.session_state.custom_query_text.strip(), flags=re.IGNORECASE).rstrip('; ') + " LIMIT 100"
                df_initial = pd.read_sql_query(initial_query, conn)
    ...
    if not get_table_names(conn):
        ...
        try:
            # 전체 데이터를 로드하지 않고 컬럼 목록만 가져오기 위해 LIMIT 1 사용
            df_cols = pd.read_sql_query(f"SELECT * FROM {selected_table} LIMIT 1", conn)
            ...
            if selected_columns:
                ...
                query_filtered = f"SELECT {columns_str} FROM {selected_table} LIMIT 100" # 100개만 조회
                df_filtered = pd.read_sql_query(query_filtered, conn)
                
                with col_display:
                    st.markdown(f"**테이블 `{selected_table}` 조회 결과 (선택 필드, 100개 제한)**")
