...
DB_FILE = 'product_history_analysis.db' 
BASE_MEASUREMENTS = [
...
SELECT * FROM (
    WITH All_Test_Results AS (
        ...
        -- ⭐⭐⭐ 날짜 필터 추가 ⭐⭐⭐
        WHERE T1.PcbStartTime >= '2025-09-20' AND T1.PcbStartTime < '2025-09-21'
    ),
...
selected_mode = st.radio( 
    "작업 모드 선택:",
    ['1. 데이터 저장/추가 (CSV Insert)', '2. DB 조회 및 분석', '3. 주차별 DB 데이터 삭제'],
...
# ==============================================================================
    ...
    df_status = get_db_status(conn)
    st.subheader("DB에 저장된 주차별 상세 데이터 현황")
    if not df_status.empty:
        st.dataframe(df_status, use_container_width=True)
        # 💡 모드 전환 시 초기 쿼리 실행
